...
// Structure to hold index key-value pair
typedef struct index_pair {
    int key;        // employee ID
    int position;   // position in data file
} index_pair;

class employeeDatabase {
    string data_file_name = "ind_employee_data.dat";     // file to store employee data
    string index_file_name = "ind_employee_index.dat";   // file to store employee index
public:
    employeeDatabase();  // constructor to initialize files
...
// Read employee from specific byte position in the data file
...
void employeeDatabase::addEmployee(int eid, char name[50], char dest[50], int sal) {
    ...
    int pos, ipos;
    bool update = false;
// Check if employee already exists or marked deleted
 ...
 // Fill employee struct
    ...
    emp.salary = sal;
 // Write employee to data file and get position
    ...
    if (update) {
        ...
        index_file.clear();
        index_file.close();
// Write all back in sorted order
...
// Check if employee exists in index
